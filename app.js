...
const bettingSites = [
  ...
  { key: 'betcris', name: 'Betcris', short: 'Betcris', url: 'https://be.betcris.do/category/flat/FCE76739-FFAC-4FFC-94D0-EEEE19F837FB', color: '#ef233c', access: 'Login asistido solo lectura' },
];
const state = {
  section: 'dashboard',
  sport: 'Todos',
  ...
  oddsError: '',
  lastUpdated: '',
  betcris: { status: 'idle', message: 'Betcris no conectado', isLoading: false, lastUpdated: '', eventsLoaded: 0 },
  clients: [
...
async function refreshBetcrisStatus() {
  try {
    const response = await fetch('/api/betcris/status', { cache: 'no-store' });
    if (!response.ok) throw new Error(`Betcris status ${response.status}`);
    applyBetcrisStatus(await response.json());
    render();
  } catch (error) {
    state.betcris = { ...state.betcris, status: 'error', message: error.message, isLoading: false };
    render();
  }
}
async function loginBetcris(username, password) {
  state.betcris = { ...state.betcris, isLoading: true, message: 'Iniciando sesión Betcris...' };
  render();
  try {
    const response = await fetch('/api/betcris/login', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ username, password }),
    });
    const payload = await response.json();
    if (!response.ok) throw new Error(payload.error || `Betcris login ${response.status}`);
    applyBetcrisStatus(payload);
    closeModal();
    await loadBetcrisOdds(true);
  } catch (error) {
    state.betcris = { ...state.betcris, status: 'login_error', message: error.message, isLoading: false };
    render();
  }
}
async function loadBetcrisOdds(force = false) {
  state.betcris = { ...state.betcris, isLoading: true, message: 'Leyendo cuotas Betcris...' };
  render();
  try {
    const response = await fetch(`/api/betcris/pregame-events${force ? '?force=1' : ''}`, { cache: 'no-store' });
    const payload = await response.json();
    if (!response.ok) throw new Error(payload.error || `Betcris odds ${response.status}`);
    applyBetcrisStatus(payload.status || {});
    if (payload.events?.length) {
      mergeProviderEvents(payload.events, 'betcris');
      state.dataSource = `${state.dataSource} + Betcris`;
      state.lastUpdated = new Date().toLocaleString('es-DO', { timeZone: 'America/Santo_Domingo' });
    }
    render();
  } catch (error) {
...
function eventSignature(eventName) {
...
function mergeProviderEvents(providerEvents, providerKey) {
  for (const providerEvent of providerEvents) {
    const existing = state.prematchEvents.find((event) => eventSignature(event.event) === eventSignature(providerEvent.event));
    if (!existing) {
      state.prematchEvents.push(providerEvent);
      continue;
    }

    for (const providerSelection of providerEvent.selections || []) {
      const existingSelection = existing.selections.find((selection) => eventSignature(selection.name) === eventSignature(providerSelection.name));
      if (existingSelection) {
        existingSelection.oddsBySite[providerKey] = providerSelection.oddsBySite[providerKey];
      } else {
        existing.selections.push(providerSelection);
      }
    }
  }
}
function applyBetcrisStatus(statusPayload) {
  state.betcris = {
    ...state.betcris,
    status: statusPayload.status || state.betcris.status,
    message: statusPayload.message || state.betcris.message,
    lastUpdated: statusPayload.lastUpdated || state.betcris.lastUpdated,
    eventsLoaded: statusPayload.eventsLoaded ?? state.betcris.eventsLoaded,
    isLoading: false,
  };
}
...
function getSelection(eventId, index) {
  const event = state.prematchEvents.find((item) => String(item.id) === String(eventId));
    ...
    <section class="compare-layout">
      ...
      <div class="compare-board">
        <div class="compare-toolbar panel">
          ...
          <button class="refresh-button" data-refresh-odds ${state.isLoadingOdds ? 'disabled' : ''}>${state.isLoadingOdds ? 'Cargando...' : 'Actualizar Sportider'}</button><button class="refresh-button betcris" data-betcris-login>Login Betcris</button><button class="refresh-button betcris" data-refresh-betcris ${state.betcris.isLoading ? 'disabled' : ''}>${state.betcris.isLoading ? 'Leyendo...' : 'Actualizar Betcris'}</button>
        ...
        <p class="notice">Aviso: el sistema recomienda y registra. No inicia sesión ni ejecuta apuestas automáticamente; el encargado coloca la jugada manualmente en la página recomendada.</p>${state.oddsError ? `<p class="notice error">${escapeHtml(state.oddsError)}</p>` : ''}<p class="notice betcris-status"><strong>Betcris:</strong> ${escapeHtml(state.betcris.message)}${state.betcris.lastUpdated ? ` · ${new Date(state.betcris.lastUpdated).toLocaleString('es-DO', { timeZone: 'America/Santo_Domingo' })}` : ''}</p>
...
function bindViewEvents() {
  ...
  document.querySelectorAll('[data-betcris-login]').forEach((button) => button.addEventListener('click', openBetcrisLoginModal));
  document.querySelectorAll('[data-refresh-betcris]').forEach((button) => button.addEventListener('click', () => loadBetcrisOdds(true)));
...
function openBetcrisLoginModal() {
  const modal = $('#modalRoot');
  modal.innerHTML = `<div class="modal-backdrop"><form class="modal-card bet-form" id="betcrisLoginForm"><div class="form-head"><h2>Login asistido Betcris</h2><button type="button" data-close>✕</button></div><p class="notice">La contraseña se envía solo al servidor local para abrir una sesión de lectura. No se guarda en app.js, localStorage ni Git. No se colocan apuestas.</p><div class="form-grid"><input name="username" autocomplete="username" placeholder="Usuario Betcris" required><input name="password" autocomplete="current-password" type="password" placeholder="Contraseña Betcris" required></div><button class="primary-button">Iniciar sesión y leer odds</button></form></div>`;
  ...
  modal.querySelector('#betcrisLoginForm').addEventListener('submit', (event) => {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
    loginBetcris(form.get('username'), form.get('password'));
  });
}
